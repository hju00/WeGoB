...
class UserSolution {
    ...
    // 참고
    // https://velog.io/@ejjem/%EB%8B%A4%EC%9D%B5%EC%8A%A4%ED%8A%B8%EB%9D%BC-%EC%95%8C%EA%B3%A0%EB%A6%AC%EC%A6%98dijkstra-algorithm
    private void dijkstra(long[] dist, int[] starts, int size, int limit) {
        ...
        while (!pq.isEmpty()) {
            ...
            if (curDist != dist[cur])
                continue; // 이미 갱신된 최단거리 있으면 더 볼 필요 없음
            if (curDist > limit)
                break; // 이제 나올 애들은 볼 필요 없음
